...
var _ = Describe("Webhook URLs", func() {
	...
	Context("With a webhook token", func() {
		...
		It("should hand out the token to Tetragon and Kive", func() {
			...
			tracingPolicy := generateTetragonTracingPolicy(context.Background(), &deceptionPolicy, helpersTraps[0], "test-tracing-policy")
			...
			kivePolicy := generateKivePolicy(context.Background(), &deceptionPolicy, helpersTraps[0], "test-kive-policy")
